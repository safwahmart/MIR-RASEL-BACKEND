...
import DeliveryManAdd from './DeliveryManAdd';
...
const CategoriesTypeList = () => {
    ...
    const handleConfirmationDialog = useConfirmationDialog(
        language === 'en-US' ? "Delete Attribute Delivery Man?" : "ট্যাগ মুছবেন?",
    ...
    const handleFetchCategoriesType = React.useCallback(
        async () => {
            try {
                const data = await axios.get(`${baseUrl}/deliveryMans`, {
                    headers: headers
                });
    ...
    const deleteDeliveryMan = async (id) => {
        try {
            ...
            if (confirmed) {
                ...
                const result = await axios.delete(`${baseUrl}/deliveryMans/${id}`, {
                    headers: headers,
                });
    ...
    const handleStatusUpdate = (row) => {
        ...
        formData.append('delivery_man_name', row.delivery_man_name);
        formData.append('status', row.status === 0 ? 1 : 0);
        formData.append('_method', 'PUT');
        axios
            .post(
                `${baseUrl}/deliveryMans/` + row.id, formData,
            ...
            .then(function (response) {
                showToast(
                    response.data.data.status == 1 ? "Delivery Man  enable" : "Delivery Man Disable ",
                    "success"
                );
    ...
    return (
        <MainCard title={t("Delivery Man List")} content={false}>
            <CardContent>
                <Grid container justifyContent="space-between" alignItems="center" spacing={2}>
                    ...
                    <Grid item xs={12} sm={6} sx={{ textAlign: 'right' }}>
                        ...
                        {/* product add & dialog */}
                        <Tooltip title={t("Add Delivery Man")}>
                            <Fab
                                color="primary"
                                size="small"
                        ...
                        <DeliveryManAdd handelFatchDeliveryMan={handelFatchDeliveryMan} open={open} handleCloseDialog={handleCloseDialog} selectedData={selectedData} />
                    </Grid>
                </Grid>
            </CardContent>
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            >
                                                {' '}
                                                {language === 'bn' ? row.delivery_type_id === 1 ? 'Courier' : 'Person' : row.delivery_type_id === 1 ? 'Courier' : 'Person'}{' '}
                                            </Typography>
                                        </TableCell>
                                        <TableCell
                                            component="th"
                                        ...
                                        >
                                            ...
                                            >
                                                {' '}
                                                {language === 'bn' ? row.delivery_man_name_bn : row.delivery_man_name}{' '}
                                            </Typography>
                                        </TableCell>
                                        <TableCell
                                            component="th"
                                        ...
                                        >
                                            ...
                                            >
                                                {' '}
                                                {language === 'bn' ? row.district_name_bn : row.district_name}{' '}
                                            </Typography>
                                        </TableCell>
                                        <TableCell
                                            component="th"
                                        ...
                                        >
                                            ...
                                            >
                                                {' '}
                                                {language === 'bn' ? row.area_name_bn : row.area_name}{' '}
                                            </Typography>
                                        </TableCell>
                                        <TableCell
                                            component="th"
                                        ...
                                        >
                                            ...
                                            >
                                                {' '}
                                                {language === 'bn' ? toBengaliNumber(row.phone) : row.phone}{' '}
                                            </Typography>
                                        </TableCell>
                                        <TableCell
                                            component="th"
                                        ...
                                        >
                                            ...
                                            >
                                                {' '}
                                                {language === 'bn' ? toBengaliNumber(row.email) : row.email}{' '}
                                            </Typography>
                                        </TableCell>
