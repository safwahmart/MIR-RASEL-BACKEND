...
import AccountControlAdd from './AccountGroupAdd';
...
const CategoriesTypeList = () => {
    ...
    const handleConfirmationDialog = useConfirmationDialog(
        language === 'en-US' ? "Delete AccountControl?" : "অ্যাট্রিবিউট মুছবেন?",
    ...
    const handleFetchCategoriesType = React.useCallback(
        async () => {
            try {
                const data = await axios.get(`${baseUrl}/accountSubsidarys`, {
                    headers: headers
                });
                // debugger;
                setCategoriesTypeList(data?.data);
                setRows(data?.data)
    ...
    const deleteAccountControls = async (id) => {
        try {
    ...
    const handleStatusUpdate = (row) => {
        ...
        axios
            ...
            .then(function (response) {
                showToast(
                    response.data.data.status == 1 ? "AccountControls  enable" : "AccountControls Disable ",
                    "success"
                );
                handelFatchAccountControls()
            })
    ...
    const handelFatchAccountControls = () => {
        setFetch(true);
    }
    ...
    return (
        <MainCard title={t("Account Subsidary List")} content={false}>
            <CardContent>
                <Grid container justifyContent="space-between" alignItems="center" spacing={2}>
                    ...
                    <Grid item xs={12} sm={6} sx={{ textAlign: 'right' }}>
                        ...
                        {/* product add & dialog */}
                        {/* <Tooltip title={t("Add Account Group")}>
                            <Fab
                                color="primary"
                                size="small"
                        ...
                        <AccountControlAdd handelFatchAccountControls={handelFatchAccountControls} open={open} handleCloseDialog={handleCloseDialog} selectedData={selectedData} />
                    </Grid>
                </Grid>
            </CardContent>
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            >
                                                {' '}
                                                {language === 'bn' ? row.group_name_bn : row.group_name}{' '}
                                            </Typography>
                                        </TableCell>
                                        <TableCell
                                            component="th"
                                        ...
                                        >
                                            ...
                                            >
                                                {' '}
                                                {language === 'bn' ? row.control_name_bn : row.control_name}{' '}
                                            </Typography>
                                        </TableCell>
                                        <TableCell
                                            component="th"
                                        ...
                                        >
                                            ...
                                            >
                                                {' '}
                                                {language === 'bn' ? row.sub_name_bn : row.sub_name}{' '}
                                            </Typography>
                                        </TableCell>
