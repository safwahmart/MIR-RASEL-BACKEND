...
import DeliveryManAdd from './PageAdd';
...
const CategoriesTypeList = () => {
    ...
    const deleteDeliveryMan = async (id) => {
        try {
            ...
            if (confirmed) {
                ...
                const result = await axios.delete(`${baseUrl}/pages/${id}`, {
                    headers: headers,
                });
    ...
    const handleStatusUpdate = (row) => {
        ...
        formData.append('page_name', row.page_name);
        formData.append('status', row.status === 0 ? 1 : 0);
        formData.append('_method', 'PUT');
        axios
            .post(
                `${baseUrl}/pages/` + row.id, formData,
            ...
            .then(function (response) {
                showToast(
                    response.data.data.status == 1 ? "Pages  enable" : "Pages Disable ",
                    "success"
                );
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            >
                                                {' '}
                                                {language === 'bn' ? row.page_name : row.page_name}{' '}
                                            </Typography>
                                        </TableCell>
