...
const CategoriesTypeList = () => {
    ...
    const handleConfirmationDialog = useConfirmationDialog(
        language === 'en-US' ? "Delete Attribute?" : "অ্যাট্রিবিউট মুছবেন?",
    ...
    const handleFetchCategoriesType = React.useCallback(
        async () => {
            try {
                const data = await axios.get(`${baseUrl}/attributes`, {
                    headers: headers
                });
    ...
    const deleteAttributes = async (id) => {
        try {
            ...
            if (confirmed) {
                ...
                const result = await axios.delete(`${baseUrl}/attributes/${id}`, {
                    headers: headers,
                });
    ...
    const handleStatusUpdate = (row) => {
        ...
        axios
            .post(
                `${baseUrl}/attributes/` + row.id, formData,
            ...
            .then(function (response) {
                showToast(
                    response.data.data.status == 1 ? "Attributes  enable" : "Attributes Disable ",
                    "success"
                );
    ...
    return (
        <MainCard title={t("Attributes List")} content={false}>
            <CardContent>
                <Grid container justifyContent="space-between" alignItems="center" spacing={2}>
                    ...
                    <Grid item xs={12} sm={6} sx={{ textAlign: 'right' }}>
                        ...
                        <AttributeAdd handelFatchAttributes={handelFatchAttributes} open={open} handleCloseDialog={handleCloseDialog} selectedData={selectedData} />
                    </Grid>
                </Grid>
            </CardContent>
                ...
                <Table sx={{ minWidth: 750 }} aria-labelledby="tableTitle">
                    ...
                    <TableBody>
                        {stableSort(rows, getComparator(order, orderBy))
                            ...
                            .map((row, index) => {
                                console.log('attributeTypes', row);
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <TableCell
                                            ...
                                            id={labelId}
                                            scope="row"
                                            onClick={(event) => handleClick(event, row.color_code)}
                                            sx={{ cursor: 'pointer' }}
                                        >
                                            <Typography
                                                variant="subtitle1"
