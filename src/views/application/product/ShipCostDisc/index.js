...
import TimeSlotAdd from './ShipCostDiscAdd';
...
const CategoriesTypeList = () => {
  ...
  const handleConfirmationDialog = useConfirmationDialog(
    language === 'en-US'?"Delete Shipping Cost Discount?":"অ্যাট্রিবিউটের ধরন মুছবেন?",
  ...
  const handleFetchCategoriesType = React.useCallback(
    async () => {
      ...
      try {
        const data = await axios.get(`${baseUrl}/shippingCostDiscounts`, {
          headers: headers
        });
  ...
  const deleteTimeSlots = async (id) => {
    try {
      ...
      if (confirmed) {
        ...
        const result = await axios.delete(`${baseUrl}/shippingCostDiscounts/${id}`, {
          headers: headers,
        });
  ...
  const handleStatusUpdate = (row) => {
    ...
    formData.append('from_amount', row.from_amount);
    formData.append('status', row.status === 0 ? 1 : 0);
    formData.append('_method', 'PUT');
    axios
      .post(
        `${baseUrl}/shippingCostDiscounts/` + row.id, formData,
      ...
      .then(function (response) {
        showToast(
          response.data.data.status === 1 ? "Shipping Cost Discount enable" : "Shipping Cost Discount Disable ",
          "success"
        );
  ...
  const handelFetchShippingCostDisc = () => {
    setFetch(true);
  }
  ...
  return (
    <MainCard title={t('Shipping Cost Discount List')} content={false}>
      <CardContent>
        <Grid container justifyContent="space-between" alignItems="center" spacing={2}>
          ...
          <Grid item xs={12} sm={6} sx={{ textAlign: 'right' }}>
            ...
            {/* product add & dialog */}
            <Tooltip title={t('Add Shipping Cost Discount')}>
              <Fab
                color="primary"
                size="small"
            ...
            <TimeSlotAdd handelFetchShippingCostDisc={handelFetchShippingCostDisc} open={open} handleCloseDialog={handleCloseDialog} selectedData={selectedData} />
          </Grid>
        </Grid>
      </CardContent>
                ...
                return (
                  ...
                  >
                    <TableCell
                      ...
                      id={labelId}
                      scope="row"
                      onClick={(event) => handleClick(event, row.from_amount)}
                      sx={{ cursor: 'pointer' }}
                    >
                      <Typography
                        variant="subtitle1"
                    ...
                    >
                      ...
                      >
                        {' '}
                        {language === 'bn'?toBengaliNumber(row.from_amount):row.from_amount}{' '}
                      </Typography>
                    </TableCell>
                    <TableCell
                      component="th"
                      id={labelId}
                      scope="row"
                      onClick={(event) => handleClick(event, row.to_amount)}
                      sx={{ cursor: 'pointer' }}
                    >
                      <Typography
                        variant="subtitle1"
                      ...
                      >
                        {' '}
                        {language === 'bn'?toBengaliNumber(row.to_amount):row.to_amount}{' '}
                      </Typography>
                    </TableCell>
                    <TableCell
                      component="th"
                      id={labelId}
                      scope="row"
                      onClick={(event) => handleClick(event, row.discount)}
                      sx={{ cursor: 'pointer' }}
                    >
                      <Typography
                        variant="subtitle1"
                      ...
                      >
                        {' '}
                        {language === 'bn'?toBengaliNumber(row.discount):row.discount}{' '}
                      </Typography>
                    </TableCell>
