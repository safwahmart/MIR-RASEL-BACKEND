...
import TimeSlotAdd from './ReturnReasonAdd';
...
const CategoriesTypeList = () => {
  ...
  const handleConfirmationDialog = useConfirmationDialog(
    language === 'en-US'?"Delete Return Reason?":"অ্যাট্রিবিউটের ধরন মুছবেন?",
  ...
  const handleFetchCategoriesType = React.useCallback(
    async () => {
      ...
      try {
        const data = await axios.get(`${baseUrl}/returnReasons`, {
          headers: headers
        });
  ...
  const deleteTimeSlots = async (id) => {
    try {
      ...
      if (confirmed) {
        ...
        const result = await axios.delete(`${baseUrl}/returnReasons/${id}`, {
          headers: headers,
        });
  ...
  const handleStatusUpdate = (row) => {
    ...
    axios
      .post(
        `${baseUrl}/returnReasons/` + row.id, formData,
      ...
      .then(function (response) {
        showToast(
          response.data.data.status === 1 ? "Return Reason enable" : "Return Reason Disable ",
          "success"
        );
  ...
  return (
    <MainCard title={t('Return Reason List')} content={false}>
      <CardContent>
        <Grid container justifyContent="space-between" alignItems="center" spacing={2}>
          ...
          <Grid item xs={12} sm={6} sx={{ textAlign: 'right' }}>
            ...
            {/* product add & dialog */}
            <Tooltip title={t('Add Return Reason')}>
              <Fab
                color="primary"
                size="small"
                ...
                return (
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        {' '}
                        {language === 'bn'?row.serial_no_bn:row.serial_no}{' '}
                      </Typography>
                    </TableCell>
