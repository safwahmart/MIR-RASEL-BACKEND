...
import HighlightTypeAdd from './TagAdd';
...
const CategoriesTypeList = () => {
    ...
    const handleConfirmationDialog = useConfirmationDialog(
        language === 'en-US' ? "Delete Attribute Tag?" : "ট্যাগ মুছবেন?",
    ...
    const deleteTags = async (id) => {
        try {
            ...
            if (confirmed) {
                ...
                const result = await axios.delete(`${baseUrl}/tags/${id}`, {
                    headers: headers,
                });
    ...
    const handleStatusUpdate = (row) => {
        ...
        axios
            ...
            .then(function (response) {
                showToast(
                    response.data.data.status == 1 ? "Tags  enable" : "Tags Disable ",
                    "success"
                );
                handelFatchTags()
            })
    ...
    return (
        <MainCard title={t("Tags List")} content={false}>
            <CardContent>
                <Grid container justifyContent="space-between" alignItems="center" spacing={2}>
                    ...
                    <Grid item xs={12} sm={6} sx={{ textAlign: 'right' }}>
                        ...
                        <HighlightTypeAdd handelFatchTags={handelFatchTags} open={open} handleCloseDialog={handleCloseDialog} selectedData={selectedData} />
                    </Grid>
                </Grid>
            </CardContent>
