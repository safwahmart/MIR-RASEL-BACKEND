...
const validationSchema = Yup.object().shape({
    ...
    name_bn: Yup.string().required('Tag Bn is required'),
    slug_bn: Yup.string().required('Slug Bn is required'),
    slug: Yup.string().required('Slug is required'),
});

const HighlightTypeAdd = ({ open, handleCloseDialog, handelFatchTags, selectedData }) => {
    ...
    const onSubmit = async (values) => {
        try {
            ...
            const response = Object.keys(selectedData).length > 0 ? await axios.post(`${baseUrl}/tags/${selectedData.id}`, formData, {
                headers: headers
            }) : await axios.post(`${baseUrl}/tags`, formData, {
                headers: headers
            });
        ...
        >
            {open && (
                ...
                >
                    {({ errors, touched }) => (
                        <Form>
                            <DialogTitle>{Object.keys(selectedData).length > 0 ? t('Edit') : t('Add')} {t('Your Tag')}</DialogTitle>
                            <DialogContent>
