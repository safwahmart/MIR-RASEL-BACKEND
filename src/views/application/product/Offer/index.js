...
import HighlightTypeAdd from './OfferAdd';
...
const CategoriesTypeList = () => {
    ...
    const handleConfirmationDialog = useConfirmationDialog(
        language === 'en-US' ? "Delete Offer?" : "হিসাবের মান মুছবেন?",
    ...
    const deleteOffers = async (id) => {
        try {
            ...
            if (confirmed) {
                ...
                const result = await axios.delete(`${baseUrl}/offers/${id}`, {
                    headers: headers,
                });
    ...
    const handleStatusUpdate = (row) => {
        ...
        axios
            .post(
                `${baseUrl}/offers/` + row.id, formData,
            ...
            .then(function (response) {
                showToast(
                    response.data.data.status == 1 ? "Offer  enable" : "Offer Disable ",
                    "success"
                );
                handelFatchOffers()
            })
    ...
    return (
        <MainCard title={t("Offer List")} content={false}>
            <CardContent>
                <Grid container justifyContent="space-between" alignItems="center" spacing={2}>
                    ...
                    <Grid item xs={12} sm={6} sx={{ textAlign: 'right' }}>
                        ...
                        <HighlightTypeAdd handelFatchOffers={handelFatchOffers} open={open} handleCloseDialog={handleCloseDialog} selectedData={selectedData} />
                    </Grid>
                </Grid>
            </CardContent>
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            >
                                                {' '}
                                                {language === 'bn' ? toBengaliNumber(row.serial) : row.serial}{' '}
                                            </Typography>
                                        </TableCell>
