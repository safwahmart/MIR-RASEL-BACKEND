...
const CategoriesTypeList = () => {
    ...
    const handleFetchCategoriesType = React.useCallback(
        async () => {
            try {
                const data = await axios.get(`${baseUrl}/productTypes`, {
                    headers: headers
                });
    ...
    const deleteProductTypes = async (id) => {
        try {
            ...
            if (confirmed) {
                ...
                const result = await axios.delete(`${baseUrl}/productTypes/${id}`, {
                    headers: headers
                });
    ...
    const handleStatusUpdate = (row) => {
        ...
        axios
            ...
            .then(function (response) {
                showToast(response.data.data.status == 1 ? 'Product Types  enable' : 'Product Types Disable ', 'success');
    ...
    return (
        <MainCard title={t('Product Types List')} content={false}>
            <CardContent>
                <Grid container justifyContent="space-between" alignItems="center" spacing={2}>
                    ...
                    <Grid item xs={12} sm={6} sx={{ textAlign: 'right' }}>
                        ...
                        <ProductTypeAdd
                            handelFatchProductTypes={handelFatchProductTypes}
                            open={open}
                                ...
                                return (
                                    <TableRow key={row.id}>
                                        ...
                                        <TableCell align="center" sx={{ pr: 3 }}>
                                            ...
                                            <IconButton
                                                onClick={() => deleteProductTypes(row?.id)}
                                                color="primary"
                                                size="large"
                                                aria-label="delet"
                                            >
