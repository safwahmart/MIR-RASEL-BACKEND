...
import HighlightTypeAdd from './UnitAdd';
...
const CategoriesTypeList = () => {
    ...
    const handleConfirmationDialog = useConfirmationDialog(
        language === 'en-US' ? "Delete Unit?" : "হিসাবের মান মুছবেন?",
    ...
    const handleFetchCategoriesType = React.useCallback(
        async () => {
            try {
                const data = await axios.get(`${baseUrl}/units`, {
                    headers: headers
                });
    ...
    const deleteUnits = async (id) => {
        try {
            ...
            if (confirmed) {
                ...
                const result = await axios.delete(`${baseUrl}/units/${id}`, {
                    headers: headers,
                });
    ...
    const handleStatusUpdate = (row) => {
        ...
        axios
            ...
            .then(function (response) {
                showToast(
                    response.data.data.status == 1 ? "Unit  enable" : "Unit Disable ",
                    "success"
                );
                handelFatchUnits()
            })
    ...
    return (
        <MainCard title={t("Unit List")} content={false}>
            <CardContent>
                <Grid container justifyContent="space-between" alignItems="center" spacing={2}>
                    ...
                    <Grid item xs={12} sm={6} sx={{ textAlign: 'right' }}>
                        ...
                        <HighlightTypeAdd handelFatchUnits={handelFatchUnits} open={open} handleCloseDialog={handleCloseDialog} selectedData={selectedData} />
                    </Grid>
                </Grid>
            </CardContent>
