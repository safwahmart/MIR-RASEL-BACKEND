...
const CategoriesTypeList = () => {
    ...
    const handleConfirmationDialog = useConfirmationDialog(
        language === 'en-US' ? 'Delete Brands?' : 'ব্র্যান্ড মুছবেন?',
    ...
    const deleteBrands = async (id) => {
        try {
            ...
            if (confirmed) {
                ...
                const result = await axios.delete(`${baseUrl}/brands/${id}`, {
                    headers: headers
                });
    ...
    const handleShowMenuUpdate = (row) => {
        ...
        axios
            ...
            .then(function (response) {
                showToast(response.data.data.show_menu == 1 ? ' Show Menu enable' : 'Show Menu Disable ', 'success');
                handelFatchBrands();
            })
            .catch(function (error) {
                showToast('Something went wrong!', 'error');
    ...
    const handleHighlightUpdate = (row) => {
        ...
        axios
            ...
            .then(function (response) {
                showToast(response.data.data.highlight == 1 ? 'Highlight enable' : 'Highlight Disable ', 'success');
                handelFatchBrands();
            })
            .catch(function (error) {
                showToast('Something went wrong!', 'error');
    ...
    return (
        <MainCard title={t('Brands List')} content={false}>
            <CardContent>
                <Grid container justifyContent="space-between" alignItems="center" spacing={2}>
                    <Grid item xs={12} sm={6}></Grid>
                    <Grid item xs={12} sm={6} sx={{ textAlign: 'right' }}>
                        {/* product add & dialog */}
                        <Tooltip title={t('Add Brands')}>
                            <Fab
                                color="primary"
                                size="small"
                        ...
                        <BrandsAdd
                            handelFatchBrands={handelFatchBrands}
                            open={open}
