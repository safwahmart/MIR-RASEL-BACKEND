...
const CategoriesTypeList = () => {
    ...
    const handleConfirmationDialog = useConfirmationDialog(
        language === 'en-US' ? "Delete Attribute Discount Type?" : "ট্যাগ মুছবেন?",
    ...
    const handleFetchCategoriesType = React.useCallback(
        async () => {
            try {
                const data = await axios.get(`${baseUrl}/discounts`, {
                    headers: headers
                });
    ...
    const deleteDiscount = async (id) => {
        try {
            ...
            if (confirmed) {
                ...
                const result = await axios.delete(`${baseUrl}/discounts/${id}`, {
                    headers: headers,
                });
    ...
    const handleStatusUpdate = (row) => {
        ...
        formData.append('offer_id', row.offer_id);
        formData.append('offer_status', row.offer_status === 0 ? 1 : 0);
        formData.append('_method', 'PUT');
        axios
            .post(
                `${baseUrl}/discounts/` + row.id, formData,
            ...
            .then(function (response) {
                showToast(
                    response.data.data.offer_status === 1 ? "Discount  enable" : "Discount Disable ",
                    "success"
                );
                handelFatchDiscount()
            })
    ...
    return (
        <MainCard title={t("Discount List")} content={false}>
            <CardContent>
                <Grid container justifyContent="space-between" alignItems="center" spacing={2}>
                    ...
                    <Grid item xs={12} sm={6} sx={{ textAlign: 'right' }}>
                        ...
                        <DiscountEdit handelFatchDiscount={handelFatchDiscount} open={open} handleCloseDialog={handleCloseDialog} selectedData={selectedData} />
                    </Grid>
                </Grid>
            </CardContent>
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            >
                                                {' '}
                                                {language === 'bn' ? toBengaliNumber(row.discount_percent) : row.discount_percent}{' '}
                                            </Typography>
                                        </TableCell>
                                        <TableCell
                                            component="th"
                                        ...
                                        >
                                            ...
                                            >
                                                {' '}
                                                {language === 'bn' ? toBengaliNumber(row.discount_flat) : row.discount_flat}{' '}
                                            </Typography>
                                        </TableCell>
                                        <TableCell
                                            component="th"
                                        ...
                                        >
                                            ...
                                            >
                                                {' '}
                                                {language === 'bn' ? toBengaliNumber(row.start_date) : row.start_date}{' '}
                                            </Typography>
                                        </TableCell>
                                        <TableCell
                                            component="th"
                                        ...
                                        >
                                            ...
                                            >
                                                {' '}
                                                {language === 'bn' ? toBengaliNumber(row.end_date) : row.end_date}{' '}
                                            </Typography>
                                        </TableCell>
                                        <TableCell align="center">
                                            <Switch
                                                defaultChecked={row.offer_status === 0 ? false : true}
