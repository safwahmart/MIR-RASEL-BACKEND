...
import TimeSlotAdd from './ExtraShipCostAdd';
...
const CategoriesTypeList = () => {
  ...
  const handleConfirmationDialog = useConfirmationDialog(
    language === 'en-US'?"Delete Extra Shipping Cost?":"অ্যাট্রিবিউটের ধরন মুছবেন?",
  ...
  const handleFetchCategoriesType = React.useCallback(
    async () => {
      ...
      try {
        const data = await axios.get(`${baseUrl}/extraShippCosts`, {
          headers: headers
        });
  ...
  const deleteTimeSlots = async (id) => {
    try {
      ...
      if (confirmed) {
        ...
        const result = await axios.delete(`${baseUrl}/extraShippCosts/${id}`, {
          headers: headers,
        });
  ...
  const handleStatusUpdate = (row) => {
    ...
    formData.append('from_weight', row.from_weight);
    formData.append('status', row.status === 0 ? 1 : 0);
    formData.append('_method', 'PUT');
    axios
      .post(
        `${baseUrl}/extraShippCosts/` + row.id, formData,
      ...
      .then(function (response) {
        showToast(
          response.data.data.status === 1 ? "Extra Shipping Cost enable" : "Extra Shipping Cost Disable ",
          "success"
        );
  ...
  const handelFetchExtraShipCost = () => {
    setFetch(true);
  }
  ...
  return (
    <MainCard title={t('Extra Shipping Cost List')} content={false}>
      <CardContent>
        <Grid container justifyContent="space-between" alignItems="center" spacing={2}>
          ...
          <Grid item xs={12} sm={6} sx={{ textAlign: 'right' }}>
            ...
            {/* product add & dialog */}
            <Tooltip title={t('Add Extra Shipping Cost')}>
              <Fab
                color="primary"
                size="small"
            ...
            <TimeSlotAdd handelFetchExtraShipCost={handelFetchExtraShipCost} open={open} handleCloseDialog={handleCloseDialog} selectedData={selectedData} />
          </Grid>
        </Grid>
      </CardContent>
                ...
                return (
                  ...
                  >
                    <TableCell
                      ...
                      id={labelId}
                      scope="row"
                      onClick={(event) => handleClick(event, row.from_weight)}
                      sx={{ cursor: 'pointer' }}
                    >
                      <Typography
                        variant="subtitle1"
                    ...
                    >
                      ...
                      >
                        {' '}
                        {language === 'bn'?toBengaliNumber(row.from_weight):row.from_weight}{' '}
                      </Typography>
                    </TableCell>
                    <TableCell
                      component="th"
                      id={labelId}
                      scope="row"
                      onClick={(event) => handleClick(event, row.to_weight)}
                      sx={{ cursor: 'pointer' }}
                    >
                      <Typography
                        variant="subtitle1"
                      ...
                      >
                        {' '}
                        {language === 'bn'?toBengaliNumber(row.to_weight):row.to_weight}{' '}
                      </Typography>
                    </TableCell>
                    <TableCell
                      component="th"
                      id={labelId}
                      scope="row"
                      onClick={(event) => handleClick(event, row.extra_cost)}
                      sx={{ cursor: 'pointer' }}
                    >
                      <Typography
                        variant="subtitle1"
                      ...
                      >
                        {' '}
                        {language === 'bn'?toBengaliNumber(row.extra_cost):row.extra_cost}{' '}
                      </Typography>
                    </TableCell>
