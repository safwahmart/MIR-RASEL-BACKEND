...
import TimeSlotAdd from './PointAdd';
...
const CategoriesTypeList = () => {
  ...
  const handleConfirmationDialog = useConfirmationDialog(
    language === 'en-US'?"Delete Point?":"অ্যাট্রিবিউটের ধরন মুছবেন?",
  ...
  const handleFetchCategoriesType = React.useCallback(
    async () => {
      ...
      try {
        const data = await axios.get(`${baseUrl}/points`, {
          headers: headers
        });
  ...
  const deleteTimeSlots = async (id) => {
    try {
      ...
      if (confirmed) {
        ...
        const result = await axios.delete(`${baseUrl}/points/${id}`, {
          headers: headers,
        });
  ...
  const handleStatusUpdate = (row) => {
    ...
    axios
      .post(
        `${baseUrl}/points/` + row.id, formData,
      ...
      .then(function (response) {
        showToast(
          response.data.data.status === 1 ? "Point enable" : "Point Disable ",
          "success"
        );
  ...
  const handelFetchReturnReasons = () => {
    setFetch(true);
  }
  ...
  return (
    <MainCard title={t('Point List')} content={false}>
      <CardContent>
        <Grid container justifyContent="space-between" alignItems="center" spacing={2}>
          ...
          <Grid item xs={12} sm={6} sx={{ textAlign: 'right' }}>
            ...
            <TimeSlotAdd handelFetchReturnReasons={handelFetchReturnReasons} open={open} handleCloseDialog={handleCloseDialog} selectedData={selectedData} />
          </Grid>
        </Grid>
      </CardContent>
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell
                      ...
                      id={labelId}
                      scope="row"
                      onClick={(event) => handleClick(event, row.max_purchase_amount)}
                      sx={{ cursor: 'pointer' }}
                    >
                      <Typography
                        variant="subtitle1"
                      ...
                      >
                        {' '}
                        {language === 'bn'?toBengaliNumber(row.max_purchase_amount):row.max_purchase_amount}{' '}
                      </Typography>
                    </TableCell>
                    <TableCell
                      component="th"
                      id={labelId}
                      scope="row"
                      onClick={(event) => handleClick(event, row.min_purchase_amount)}
                      sx={{ cursor: 'pointer' }}
                    >
                      <Typography
                        variant="subtitle1"
                      ...
                      >
                        {' '}
                        {language === 'bn'?toBengaliNumber(row.min_purchase_amount):row.min_purchase_amount}{' '}
                      </Typography>
                    </TableCell>
                    <TableCell
                      component="th"
                      id={labelId}
                      scope="row"
                      onClick={(event) => handleClick(event, row.point)}
                      sx={{ cursor: 'pointer' }}
                    >
                      <Typography
                        variant="subtitle1"
                      ...
                      >
                        {' '}
                        {language === 'bn'?toBengaliNumber(row.point):row.point}{' '}
                      </Typography>
                    </TableCell>
