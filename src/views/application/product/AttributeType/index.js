...
import AttributeTypeAdd from './AttributeTypeAdd';
...
const CategoriesTypeList = () => {
  ...
  const handleConfirmationDialog = useConfirmationDialog(
    language === 'en-US' ? "Delete Attribute Types?" : "অ্যাট্রিবিউটের ধরন মুছবেন?",
  ...
  const handleFetchCategoriesType = React.useCallback(
    async () => {
      ...
      try {
        const data = await axios.get(`${baseUrl}/attributeTypes`, {
          headers: headers
        });
  ...
  const deleteAttributeTypes = async (id) => {
    try {
      ...
      if (confirmed) {
        ...
        const result = await axios.delete(`${baseUrl}/attributeTypes/${id}`, {
          headers: headers,
        });
  ...
  const handleStatusUpdate = (row) => {
    ...
    axios
      .post(
        `${baseUrl}/attributeTypes/` + row.id, formData,
      ...
      .then(function (response) {
        showToast(
          response.data.data.status == 1 ? "Attribute Types  enable" : "Attribute Types Disable ",
          "success"
        );
  ...
  const handelFatchAttributeTypes = () => {
    setFetch(true);
  }
  ...
  return (
    <MainCard title={t('attribute type list')} content={false}>
      <CardContent>
        <Grid container justifyContent="space-between" alignItems="center" spacing={2}>
          ...
          <Grid item xs={12} sm={6} sx={{ textAlign: 'right' }}>
            ...
            {/* product add & dialog */}
            <Tooltip title={t('Add Attribute Type')}>
              <Fab
                color="primary"
                size="small"
            ...
            <AttributeTypeAdd handelFatchAttributeTypes={handelFatchAttributeTypes} open={open} handleCloseDialog={handleCloseDialog} selectedData={selectedData} />
          </Grid>
        </Grid>
      </CardContent>
                ...
                return (
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        {' '}
                        {language === 'bn' ? row.attribute_name_bn : row.attribute_name}{' '}
                      </Typography>
                    </TableCell>
                    <TableCell
                      component="th"
                    ...
                    >
                      ...
                      >
                        {' '}
                        {language === 'bn' ? row.description_bn : row.description}{' '}
                      </Typography>
                    </TableCell>
