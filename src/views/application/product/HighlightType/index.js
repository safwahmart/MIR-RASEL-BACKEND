...
import HighlightTypeAdd from './HighlightTypeAdd';
...
const CategoriesTypeList = () => {
    ...
    const handleConfirmationDialog = useConfirmationDialog(
        language === 'en-US' ? "Delete Highlight Types?" : "হাইলাইট টাইপ মুছবেন?",
    ...
    const handleFetchCategoriesType = React.useCallback(
        async () => {
            try {
                const data = await axios.get(`${baseUrl}/highlightTypes`, {
                    headers: headers
                });
    ...
    const deleteHighlightTypes = async (id) => {
        try {
            ...
            if (confirmed) {
                ...
                const result = await axios.delete(`${baseUrl}/highlightTypes/${id}`, {
                    headers: headers,
                });
    ...
    const handleStatusUpdate = (row) => {
        ...
        axios
            .post(
                `${baseUrl}/highlightTypes/` + row.id, formData,
            ...
            .then(function (response) {
                showToast(
                    response.data.data.status == 1 ? "Highlight Types  enable" : "Highlight Types Disable ",
                    "success"
                );
    ...
    const handelFatchHighlightTypes = () => {
        setFetch(true);
    }

    const redirectToNewRoute = (id) => {
        console.log('Redirecting to', id);
        navigate(`/attribute_type/edit?id=${id}`);
    }
    ...
    return (
        <MainCard title={t("Highlight Types List")} content={false}>
            <CardContent>
                <Grid container justifyContent="space-between" alignItems="center" spacing={2}>
                    ...
                    <Grid item xs={12} sm={6} sx={{ textAlign: 'right' }}>
                        ...
                        {/* product add & dialog */}
                        <Tooltip title={t("Add Highlight Type")}>
                            <Fab
                                color="primary"
                                size="small"
                        ...
                        <HighlightTypeAdd handelFatchHighlightTypes={handelFatchHighlightTypes} open={open} handleCloseDialog={handleCloseDialog} selectedData={selectedData} />
                    </Grid>
                </Grid>
            </CardContent>
