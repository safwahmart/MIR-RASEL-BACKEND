...
import TimeSlotAdd from './TimeSlotAdd';
...
const CategoriesTypeList = () => {
  ...
  const handleConfirmationDialog = useConfirmationDialog(
    language === 'en-US'?"Delete Time Slot?":"অ্যাট্রিবিউটের ধরন মুছবেন?",
  ...
  const handleFetchCategoriesType = React.useCallback(
    async () => {
      ...
      try {
        const data = await axios.get(`${baseUrl}/timeSlotes`, {
          headers: headers
        });
  ...
  const deleteTimeSlots = async (id) => {
    try {
      ...
      if (confirmed) {
        ...
        const result = await axios.delete(`${baseUrl}/timeSlotes/${id}`, {
          headers: headers,
        });
  ...
  const handleStatusUpdate = (row) => {
    ...
    formData.append('time_slot_name', row.time_slot_name);
    formData.append('status', row.status === 0 ? 1 : 0);
    formData.append('_method', 'PUT');
    axios
      .post(
        `${baseUrl}/timeSlotes/` + row.id, formData,
      ...
      .then(function (response) {
        showToast(
          response.data.data.status === 1 ? "Time Slot enable" : "Time Slot Disable ",
          "success"
        );
  ...
  return (
    <MainCard title={t('Time Slot List')} content={false}>
      <CardContent>
        <Grid container justifyContent="space-between" alignItems="center" spacing={2}>
          ...
          <Grid item xs={12} sm={6} sx={{ textAlign: 'right' }}>
            ...
            <TimeSlotAdd handelFetchTimeSlots={handelFetchTimeSlots} open={open} handleCloseDialog={handleCloseDialog} selectedData={selectedData} />
          </Grid>
        </Grid>
      </CardContent>
                ...
                return (
                  ...
                  >
                    <TableCell
                      ...
                      id={labelId}
                      scope="row"
                      onClick={(event) => handleClick(event, row.time_slot_name)}
                      sx={{ cursor: 'pointer' }}
                    >
                      <Typography
                        variant="subtitle1"
                    ...
                    >
                      ...
                      >
                        {' '}
                        {language === 'bn'?row.time_slot_name_bn:row.time_slot_name}{' '}
                      </Typography>
                    </TableCell>
                    <TableCell
                      component="th"
                      id={labelId}
                      scope="row"
                      onClick={(event) => handleClick(event, row.starting_time)}
                      sx={{ cursor: 'pointer' }}
                    >
                      <Typography
                        variant="subtitle1"
                      ...
                      >
                        {' '}
                        {language === 'bn' ? row.starting_time : row.starting_time}{' '}
                      </Typography>
                    </TableCell>
                    <TableCell
                      component="th"
                      id={labelId}
                      scope="row"
                      onClick={(event) => handleClick(event, row.ending_time)}
                      sx={{ cursor: 'pointer' }}
                    >
                      <Typography
                        variant="subtitle1"
                      ...
                      >
                        {' '}
                        {language === 'bn' ? row.ending_time : row.ending_time}{' '}
                      </Typography>
                    </TableCell>
                    <TableCell
                      component="th"
                      id={labelId}
                      scope="row"
                      onClick={(event) => handleClick(event, row.disable_at)}
                      sx={{ cursor: 'pointer' }}
                    >
                      <Typography
                        variant="subtitle1"
                      ...
                      >
                        {' '}
                        {language === 'bn' ? row.disable_at : row.disable_at}{' '}
                      </Typography>
                    </TableCell>
