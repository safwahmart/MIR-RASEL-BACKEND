...
const CategoriesTypeList = () => {
    ...
    const handleConfirmationDialog = useConfirmationDialog(
        language === 'en-US' ? "Delete Attribute Customer Type?" : "ট্যাগ মুছবেন?",
    ...
    const handleFetchCategoriesType = React.useCallback(
        async () => {
            try {
                const data = await axios.get(`${baseUrl}/customerTypes`, {
                    headers: headers
                });
    ...
    const deleteCustomerTypes = async (id) => {
        try {
            ...
            if (confirmed) {
                ...
                const result = await axios.delete(`${baseUrl}/customerTypes/${id}`, {
                    headers: headers,
                });
    ...
    const handleStatusUpdate = (row) => {
        ...
        axios
            .post(
                `${baseUrl}/customerTypes/` + row.id, formData,
            ...
            .then(function (response) {
                showToast(
                    response.data.data.status == 1 ? "Customer Types  enable" : "Customer Types Disable ",
                    "success"
                );
                handelFatchCustomerTypes()
            })
    ...
    const handelFatchCustomerTypes = () => {
        setFetch(true);
    }
    ...
    return (
        <MainCard title={t("Customer Types List")} content={false}>
            <CardContent>
                <Grid container justifyContent="space-between" alignItems="center" spacing={2}>
                    ...
                    <Grid item xs={12} sm={6} sx={{ textAlign: 'right' }}>
                        ...
                        {/* product add & dialog */}
                        <Tooltip title={t("Add Customer Type")}>
                            <Fab
                                color="primary"
                                size="small"
                        ...
                        <CustomerTypeAdd handelFatchCustomerTypes={handelFatchCustomerTypes} open={open} handleCloseDialog={handleCloseDialog} selectedData={selectedData} />
                    </Grid>
                </Grid>
            </CardContent>
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            >
                                                {' '}
                                                {language === 'bn' ? toBengaliNumber(row.percentage) : row.percentage}{' '}
                                            </Typography>
                                        </TableCell>
