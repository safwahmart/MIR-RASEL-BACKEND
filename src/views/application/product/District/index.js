...
const CategoriesTypeList = () => {
  ...
  const handleConfirmationDialog = useConfirmationDialog(
    language === 'en-US' ? "Delete Districts?" : "অ্যাট্রিবিউটের ধরন মুছবেন?",
  ...
  const handleFetchCategoriesType = React.useCallback(
    async () => {
      ...
      try {
        const data = await axios.get(`${baseUrl}/districts`, {
          headers: headers
        });
  ...
  const deleteDistricts = async (id) => {
    try {
      ...
      if (confirmed) {
        ...
        const result = await axios.delete(`${baseUrl}/districts/${id}`, {
          headers: headers,
        });
  ...
  const handleStatusUpdate = (row) => {
    ...
    axios
      .post(
        `${baseUrl}/districts/` + row.id, formData,
      ...
      .then(function (response) {
        showToast(
          response.data.data.status == 1 ? "Districts  enable" : "Districts Disable ",
          "success"
        );
  ...
  return (
    <MainCard title={t('District List')} content={false}>
      <CardContent>
        <Grid container justifyContent="space-between" alignItems="center" spacing={2}>
          ...
          <Grid item xs={12} sm={6} sx={{ textAlign: 'right' }}>
            ...
            <DistrictAdd handelFatchDistricts={handelFatchDistricts} open={open} handleCloseDialog={handleCloseDialog} selectedData={selectedData} />
          </Grid>
        </Grid>
      </CardContent>
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell
                      ...
                      id={labelId}
                      scope="row"
                      onClick={(event) => handleClick(event, row.shipping_cost)}
                      sx={{ cursor: 'pointer' }}
                    >
                      <Typography
                        variant="subtitle1"
                      ...
                      >
                        {' '}
                        {language === 'bn' ? row.shipping_cost_bn : row.shipping_cost}{' '}
                      </Typography>
                    </TableCell>
