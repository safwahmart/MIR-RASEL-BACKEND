...
const CategoriesTypeList = () => {
  ...
  const handleStatusUpdate = (row) => {
    ...
    formData.append('name_bn', row.name_bn);
    formData.append('slug', row.slug);
    formData.append('slug_bn', row.slug_bn);
    formData.append('product_type_id', row.product_type_id);
    formData.append('status', row.status === 0 ? 1 : 0);
    formData.append('_method', 'PUT');
    axios
      .post(
        `${baseUrl}/categories/` + row.id, formData,
      ...
      .then(function (response) {
        showToast(
          response.data.data.status == 1 ? "Category enable" : "Category Disable ",
          "success"
        );
        handelFatchCategory()
      })
  ...
  const handleShowMenuUpdate = (row) => {
    ...
    formData.append('slug_bn', row.slug_bn);
    formData.append('product_type_id', row.product_type_id);
    formData.append('show_menu', row.show_menu === 0 ? 1 : 0);
    formData.append('_method', 'PUT');
    axios
      .post(
        `${baseUrl}/categories/` + row.id, formData,
      ...
      .then(function (response) {
        showToast(
          response.data.data.show_menu == 1 ? " Show Menu enable" : "Show Menu Disable ",
          "success"
        );
        handelFatchCategory()
      })
  ...
  const handleHighlightUpdate = (row) => {
    ...
    axios
      ...
      .then(function (response) {
        showToast(
          response.data.data.highlight == 1 ? "Highlight enable" : "Highlight Disable ",
          "success"
        );
        handelFatchCategory()
      })
  ...
  return (
    <MainCard title={t("Category List")} content={false}>
      <CardContent>
        <Grid container justifyContent="space-between" alignItems="center" spacing={2}>
          ...
          <Grid item xs={12} sm={6} sx={{ textAlign: 'right' }}>
            ...
            <CategoryAdd handelFatchCategory={handelFatchCategory} open={open} handleCloseDialog={handleCloseDialog} selectedData={selectedData} />
          </Grid>
        </Grid>
      </CardContent>
                ...
                return (
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        {' '}
                        {language === 'bn' ? row.slug_bn : row.slug}{' '}
                      </Typography>
                    </TableCell>
                    <TableCell
                      component="th"
                    ...
                    <TableCell
                      ...
                      id={labelId}
                      scope="row"
                      onClick={(event) => handleClick(event, row.alt_text)}
                      sx={{ cursor: 'pointer' }}
                    >
                      <Typography
                        variant="subtitle1"
                      ...
                      >
                        {' '}
                        {language === 'bn' ? row.alt_text_bn : row.alt_text}{' '}
                      </Typography>
                    </TableCell>
