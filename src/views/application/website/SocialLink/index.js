...
const SocialLinkList = () => {
    ...
    const handleConfirmationDialog = useConfirmationDialog(
        language === 'en-US' ? 'Delete Social Link?' : 'সোশ্যাল লিংক মুছবেন?',
    ...
    const handleFetchSocialLink = React.useCallback(
        async () => {
            try {
                const data = await axios.get(`${baseUrl}/socialLinks`, {
                    headers: headers
                });
    ...
    const deleteSocialLinks = async (id) => {
        try {
            ...
            if (confirmed) {
                ...
                const result = await axios.delete(`${baseUrl}/socialLinks/${id}`, {
                    headers: headers
                });
    ...
    const handleStatusUpdate = (row) => {
        ...
        axios
            ...
            .then(function (response) {
                showToast(response.data.data.status == 1 ? 'Social Link enable' : 'Social Link Disable ', 'success');
                handleFetchSocials();
            })
    ...
    const handleShowMenuUpdate = (row) => {
        ...
        axios
            ...
            .then(function (response) {
                showToast(response.data.data.status == 1 ? ' Show Menu enable' : 'Show Menu Disable ', 'success');
                handleFetchSocials();
            })
            .catch(function (error) {
                showToast('Something went wrong!', 'error');
    ...
    const handleHighlightUpdate = (row) => {
        ...
        axios
            ...
            .then(function (response) {
                showToast(response.data.data.status == 1 ? 'Highlight enable' : 'Highlight Disable ', 'success');
                handleFetchSocials();
            })
            .catch(function (error) {
                showToast('Something went wrong!', 'error');
    ...
    React.useEffect(() => {
        handleFetchSocialLink();
    }, [handleFetchSocialLink]);

    return (
        <MainCard title={t('Article List')} content={false}>
            <CardContent>
                <Grid container justifyContent="space-between" alignItems="center" spacing={2}>
                    <Grid item xs={12} sm={6}></Grid>
                    <Grid item xs={12} sm={6} sx={{ textAlign: 'right' }}>
                        {/* product add & dialog */}
                        <Tooltip title={t('Add Social Link')}>
                            <Fab
                                color="primary"
                                size="small"
                        ...
                        <SocialLinksAdd
                            handleFetchSocials={handleFetchSocials}
                            open={open}
                ...
                <Table sx={{ minWidth: 750 }} aria-labelledby="tableTitle">
                    ...
                    <TableBody>
                        ...
                        <VisibilityTwoToneIcon sx={{ fontSize: '1.3rem' }} />
                                            ...
                                            <IconButton
                                                onClick={() => deleteSocialLinks(row?.id)}
                                                color="primary"
                                                size="large"
                                                aria-label="delete"
                                            >
