...
import BrandsAdd from './SlidersAdd';
...
const CategoriesTypeList = () => {
    ...
    const handleConfirmationDialog = useConfirmationDialog(
        language === 'en-US' ? 'Delete Sliders?' : 'ব্র্যান্ড মুছবেন?',
    ...
    const handleFetchCategoriesType = React.useCallback(
        async () => {
            try {
                const data = await axios.get(`${baseUrl}/sliders`, {
                    headers: headers
                });
    ...
    const deleteSliders = async (id) => {
        try {
            ...
            if (confirmed) {
                ...
                const result = await axios.delete(`${baseUrl}/sliders/${id}`, {
                    headers: headers
                });
    ...
    const handleStatusUpdate = (row) => {
        ...
        axios
            ...
            .then(function (response) {
                showToast(response.data.data.status == 1 ? 'Brand enable' : 'Brand Disable ', 'success');
                handelFatchSliders();
            })
    ...
    return (
        <MainCard title={t('Sliders List')} content={false}>
            <CardContent>
                <Grid container justifyContent="space-between" alignItems="center" spacing={2}>
                    <Grid item xs={12} sm={6}></Grid>
                    <Grid item xs={12} sm={6} sx={{ textAlign: 'right' }}>
                        {/* product add & dialog */}
                        <Tooltip title={t('Add Sliders')}>
                            <Fab
                                color="primary"
                                size="small"
                        ...
                        <BrandsAdd
                            handelFatchSliders={handelFatchSliders}
                            open={open}
                                ...
                                return (
                                    <TableRow key={row.id}>
                                        ...
                                        >
                                            <Avatar
                                                alt="product images"
                                                src={`${process.env.REACT_APP_ASSET_BASE}/uploads/${row.logo}`}
                                                size="md"
                                                variant="rounded"
                                            />
                                        </TableCell>
                                        ...
                                        >
                                            ...
                                            >
                                                {' '}
                                                {language === 'bn' ? row.button_url : row.button_url}{' '}
                                            </Typography>
                                        </TableCell>
