...
import FeedbacksAdd from './FeedbackAdd';
...
const CategoriesTypeList = () => {
  ...
  const handleConfirmationDialog = useConfirmationDialog(
    language === 'en-US' ? "Delete Feedbacks?" : "ব্র্যান্ড মুছবেন?",
  ...
  const handleFetchCategoriesType = React.useCallback(
    async () => {
      try {
        const data = await axios.get(`${baseUrl}/feedbacks`, {
          headers: headers
        });
  ...
  const deleteFeedbacks = async (id) => {
    try {
      ...
      if (confirmed) {
        ...
        const result = await axios.delete(`${baseUrl}/feedbacks/${id}`, {
          headers: headers,
        });
  ...
  const handleStatusUpdate = (row) => {
    ...
    axios
      .post(
        `${baseUrl}/feedbacks/` + row.id, formData,
      ...
      .then(function (response) {
        showToast(
          response.data.data.status == 1 ? "Feedback enable" : "Feedback Disable ",
          "success"
        );
  ...
  return (
    <MainCard title={t("Feedback List")} content={false}>
      <CardContent>
        <Grid container justifyContent="space-between" alignItems="center" spacing={2}>
          ...
          <Grid item xs={12} sm={6} sx={{ textAlign: 'right' }}>
            ...
            <FeedbacksAdd handelFatchFeedbacks={handelFatchFeedbacks} open={open} handleCloseDialog={handleCloseDialog} selectedData={selectedData} />
          </Grid>
        </Grid>
      </CardContent>
