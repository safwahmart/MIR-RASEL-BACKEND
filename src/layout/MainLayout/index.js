...
const MainLayout = () => {
    ...
    return (
        <Box sx={{ display: 'flex' }}>
            ...
            {/* header */}
            {!pathname.includes('inventory') &&<AppBar
                position="fixed"
                elevation={0}
                sx={{
            ...
            {/* drawer */}
            {(layout === LAYOUT_CONST.VERTICAL_LAYOUT || matchDownMd) && !pathname.includes('inventory') && <Sidebar />}

            {/* main content */}
            {!pathname.includes('inventory') &&<Main theme={theme} open={drawerOpen} layout={layout}>
            ...
            {pathname.includes('inventory') &&<Container maxWidth={container ? 'lg' : false} {...(!container && { sx: { px: { xs: 0 } } })}>
                    {/* breadcrumb */}
                    <Breadcrumbs separator={IconChevronRight} navigation={navigation} icon title rightAlign />
                    <Outlet />
                </Container>}
